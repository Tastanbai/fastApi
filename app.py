...
# DB_HOST = "face.tabet-kitap.kz"  # Домен или IP удалённого сервера
# DB_USER = "fastapi_user"         # Укажите имя пользователя
# DB_PASSWORD = "secure_password"  # Пароль, указанный при создании пользователя
# DB_NAME = "face_db"              # Название базы данных
...
#         """
#         cursor.execute(sql, (patient_id, hospital_id, branch_id, palata_id, image_path, emb_path))
#         conn.commit()
...
# # Функция для загрузки всех эмбеддингов из базы
...
#         conn.close()
#     except mysql.connector.Error as e:
#         raise Exception(f"Ошибка при загрузке эмбеддингов из базы: {e}")
#     return embeddings

# # Инициализируем известные эмбеддинги
# known_embeddings = load_embeddings_from_database()
...
#         # Загрузка известных эмбеддингов из базы
...
#         return {
#             "status": bool(status),
#             "similarity_percentage": float(max_similarity),
#             "embedding_time": f"{embedding_time:.2f} seconds",
...
#     # Декодируем base64 в байтовый поток
#     try:
#         image_data = base64.b64decode(file_base64)
#     except Exception as e:
#         raise HTTPException(status_code=400, detail=f"Невозможно декодировать base64: {e}")

#     # Проверка (опционально): действительно ли это изображение
#     # Можно использовать, например, imghdr или PIL
#     # import imghdr
...
#         return {
#             "message": "Данные пациента успешно обработаны (base64)",
#             "resized_image_path": resized_path,
#             "embedding_path": emb_path,
#             "embedding_time": f"{embedding_time:.2f} seconds",
#         }
